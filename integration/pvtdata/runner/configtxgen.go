...
type Configtxgen struct {
	...
	AsOrg string
	// The udo config directory
	ConfigDir string
	// The udo config directory set in the env variable
	EnvConfigDir string
...
func (c *Configtxgen) setupCommandEnv(cmd *exec.Cmd) {
	if c.EnvConfigDir != "" {
		...
		cmd.Env = append(cmd.Env, fmt.Sprintf("UDO_CFG_PATH=%s", configDir))
	}
}

// OutputBlock uses configtxgen to generate genesis block for udo.
