...
The end-to-end verification provisions a sample Udo network consisting of
...
-  Clone the Udo code base into this path.
...
.. code:: bash
       ...
       git clone https://github.com/hyperledger/udo.git
...
.. code:: bash
       ...
       # ensure sure you are in the /udo directory where the Makefile resides
       make release

This will output platform-specific binaries into the ``udo/release`` folder.

-  Next, make the Udo images.  This typically takes between five to ten minutes, so
   be patient:

.. code:: bash

       # make sure you are in the /udo directory
       make docker
...
.. code:: bash
  ...
  hyperledger/udo-couchdb     latest                e2df4dd39ca9        38 minutes ago      1.51 GB
  hyperledger/udo-couchdb     amd64-1.1.0           e2df4dd39ca9        38 minutes ago      1.51 GB
  hyperledger/udo-kafka       latest                08af4d797266        40 minutes ago      1.3 GB
  hyperledger/udo-kafka       amd64-1.1.0           08af4d797266        40 minutes ago      1.3 GB
  hyperledger/udo-zookeeper   latest                444e9e695367        40 minutes ago      1.31 GB
  hyperledger/udo-zookeeper   amd64-1.1.0           444e9e695367        40 minutes ago      1.31 GB
  hyperledger/udo-testenv     latest                8678d3101930        41 minutes ago      1.41 GB
  hyperledger/udo-testenv     amd64-1.1.0           8678d3101930        41 minutes ago      1.41 GB
  hyperledger/udo-buildenv    latest                60911392c82e        41 minutes ago      1.33 GB
  hyperledger/udo-buildenv    amd64-1.1.0           60911392c82e        41 minutes ago      1.33 GB
  hyperledger/udo-orderer     latest                2afab937b9cc        41 minutes ago      182 MB
  hyperledger/udo-orderer     amd64-1.1.0           2afab937b9cc        41 minutes ago      182 MB
  hyperledger/udo-peer        latest                9560e58e8089        41 minutes ago      185 MB
  hyperledger/udo-peer        amd64-1.1.0           9560e58e8089        41 minutes ago      185 MB
  hyperledger/udo-javaenv     latest                881ca5219fad        42 minutes ago      1.43 GB
  hyperledger/udo-javaenv     amd64-1.1.0           881ca5219fad        42 minutes ago      1.43 GB
  hyperledger/udo-ccenv       latest                28af77ffe9e9        43 minutes ago      1.29 GB
  hyperledger/udo-ccenv       amd64-1.1.0           28af77ffe9e9        43 minutes ago      1.29 GB
  hyperledger/udo-baseimage   amd64-0.4.8           f4751a503f02        3 months ago        1.27 GB
  hyperledger/udo-baseos      amd64-0.4.8           c3a4cf3b3350        3 months ago        161 MB

If you failed to compile the ``udo-testenv`` image, then you can
...
within Udo are signed by an entity's private key (``keystore``), and then verified
...
The `configtxgen
tool <https://github.com/hyperledger/udo/blob/master/docs/source/configtx.rst>`__
is used to create four artifacts: orderer **bootstrap block**, udo
...
required to successfully bootstrap a Udo network and create a channel to
transact upon.
...
.. code:: bash
  ...
  71339e7e1d38        hyperledger/udo-peer               "peer node start -..."   5 minutes ago       Up 5 minutes        0.0.0.0:8051->7051/tcp, 0.0.0.0:8053->7053/tcp     peer1.org1.example.com
  add6113ffdcf        hyperledger/udo-peer               "peer node start -..."   5 minutes ago       Up 5 minutes        0.0.0.0:10051->7051/tcp, 0.0.0.0:10053->7053/tcp   peer1.org2.example.com
  689396c0e520        hyperledger/udo-peer               "peer node start -..."   5 minutes ago       Up 5 minutes        0.0.0.0:7051->7051/tcp, 0.0.0.0:7053->7053/tcp     peer0.org1.example.com
  65424407a653        hyperledger/udo-orderer            "orderer"                5 minutes ago       Up 5 minutes        0.0.0.0:7050->7050/tcp                             orderer.example.com
  ce14853db660        hyperledger/udo-peer               "peer node start -..."   5 minutes ago       Up 5 minutes        0.0.0.0:9051->7051/tcp, 0.0.0.0:9053->7053/tcp     peer0.org2.example.com
...
blocks, as well as a state database to maintain current udo state.
...
.. code:: bash
    ...
    peer channel create -o orderer.example.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem
...
.. code:: bash
    ...
    peer chaincode install -n mycc -v 1.0 -p github.com/hyperledger/udo/examples/chaincode/go/example02/cmd
...
.. code:: bash
    ...
    peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem  -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","a","b","10"]}'
...
.. code:: bash
       ...
       peer chaincode install -o orderer.example.com:7050 -n marbles -v 1.0 -p github.com/hyperledger/udo/examples/chaincode/go/marbles02
       peer chaincode instantiate -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -v 1.0 -c '{"Args":["init"]}' -P "OR ('Org0MSP.member','Org1MSP.member')"
...
.. code:: bash
        ...
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["initMarble","marble1","blue","35","tom"]}'
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["initMarble","marble2","red","50","tom"]}'
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["initMarble","marble3","blue","70","tom"]}'
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["transferMarble","marble2","jerry"]}'
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["transferMarblesBasedOnColor","blue","jerry"]}'
        peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/udo/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/cacerts/ca.example.com-cert.pem -C $CHANNEL_NAME -n marbles -c '{"Args":["delete","marble1"]}'
...
- If you continue to see errors, share your logs on the **# udo-questions**
