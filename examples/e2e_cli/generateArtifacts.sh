...
function generateCerts (){
	CRYPTOGEN=$UDO_ROOT/release/$OS_ARCH/bin/cryptogen

	if [ -f "$CRYPTOGEN" ]; then
            echo "Using cryptogen -> $CRYPTOGEN"
	else
	    echo "Building cryptogen"
	    make -C $UDO_ROOT release
	fi
...
function generateIdemixMaterial (){
	IDEMIXGEN=$UDO_ROOT/release/$OS_ARCH/bin/idemixgen
	CURDIR=`pwd`
...
function generateChannelArtifacts() {
	...
	CONFIGTXGEN=$UDO_ROOT/release/$OS_ARCH/bin/configtxgen
